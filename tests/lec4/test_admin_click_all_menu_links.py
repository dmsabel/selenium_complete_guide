...
def test_positive(wd):
    def is_element_present(locator):
        return len(wd.find_elements_by_css_selector(locator)) > 0

    def click(css_locator):
        wd.find_element_by_css_selector(css_locator).click()
        time.sleep(1)
    ...
    li_list = wd.find_elements_by_css_selector("ul#box-apps-menu li#app-")
    ...
    for _ in li_list:
        ...
        if is_element_present(loc_ul):
            j = 1
            li_nested_list = wd.find_elements_by_css_selector(f"{loc_ul} li")
